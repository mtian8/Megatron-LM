...
def to_csr(mat: np.ndarray) -> _CSR_Matrix:
    mat_bin =(mat != 0)
    num_elms = mat_bin.astype(np.int32).sum(-1)    # number of elements in each row
    _, col_idx = np.where(mat_bin)                 # column indices of non-zero elements
    crow = np.cumsum(np.concatenate([[0],  num_elms]))  # crow[i] = first non-zero element of row i
